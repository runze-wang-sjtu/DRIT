...
if __name__ == "__main__":
    ...
    for i in range(len(real_list)):
        ...
        real_image = sitk.ReadImage(os.path.join(args.real_path, 'CT_{}'.format(mask_list[i].split('_')[2])))
        real_image = sitk.GetArrayFromImage(real_image)
        fake_image = sitk.ReadImage(os.path.join(args.fake_path, 'T1_{}'.format(mask_list[i].split('_')[2])))
