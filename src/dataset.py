...
class dataset_segmentation(data.Dataset):
    def __init__(self, opts, setname):
        ...
        self.mask_code = {0: 0, 30: 1, 60: 2, 90: 3, 120: 4}
        images = os.listdir(os.path.join(self.dataroot, opts.phase + setname))
        self.img = [os.path.join(self.dataroot, opts.phase + setname, x) for x in images]
        self.size = len(self.img)
    ...
    def __getitem__(self, index):
        ...
        label_path_name = os.path.join(self.dataroot, self.opts.phase + self.setname + '_label', label_name)
...
class dataset_unpair(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        A_case_index = data_A_name.split('_')[2]
        A_slice_index = data_A_name.split('_')[-1].split('.')[0]
        data_B_name = os.path.split(self.B[random.randint(0, self.B_size - 1)])[-1]
        B_case_index = data_B_name.split('_')[2]
        B_slice_index = self.slice_select(A_case_index, B_case_index, A_slice_index)
        PBS_B = os.path.join(os.path.split(self.B[random.randint(0, self.B_size - 1)])[0],
                             'ct_image_' + str(B_case_index) + '_' + str(B_slice_index) + '.png')
