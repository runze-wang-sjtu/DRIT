...
class Saver():
    def __init__(self, opts):
        ...
        self.n_ep_before_seg = opts.n_ep_before_seg
    ...
    def write_img(self, ep, model):
        if (ep + 1) % self.img_save_freq == 0:
            if ep < self.n_ep_before_seg:
                assembled_images = model.assemble_outputs_without_segmentation()
                img_filename = '%s/gen_%05d.jpg' % (self.image_dir, ep)
                torchvision.utils.save_image(assembled_images / 2 + 0.5, img_filename, nrow=1)
            else:
                assembled_images = model.assemble_outputs_with_segmentation()
                img_filename = '%s/gen_%05d.jpg' % (self.image_dir, ep)
                torchvision.utils.save_image(assembled_images / 2 + 0.5, img_filename, nrow=1)
